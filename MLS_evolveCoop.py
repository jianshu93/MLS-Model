...
def create_community_update_matrix(model_parameter,numGroup):
    
    r = model_parameter['r']
    mu = model_parameter['mu']
    mig = model_parameter['mig']

    numBins = model_parameter['numTypeBins']

    birthRate = r * (1-mu)
    mutationRate = r * mu / 2
    deathRate = r
    outMigrationRate = mig + mig / (numGroup-1)
    inMigrationRate = mig / (numGroup-1)


    locBirth = np.diag(np.full((numBins) , birthRate)) + \
               np.diag(np.full((numBins-1) , mutationRate), -1) + \
               np.diag(np.full((numBins-1) , mutationRate),  1)

    locDeath = np.full((numBins, numBins), deathRate) 
    locMigOut = np.diag(np.full((numBins), outMigrationRate))
    locMigIn = np.diag(np.full((numBins), inMigrationRate))

    birthMatrix = np.kron(np.eye(numGroup), locBirth)
    deathMatrix = np.kron(np.eye(numGroup), locDeath)
    migrationOutMatrix = np.kron(np.eye(numGroup), locMigOut)
    migrationInMatrix = np.kron(np.ones((numGroup,numGroup)), locMigIn)
    
    #                   np.kron(np.eye(numGroups), locMigOut)  

    birthMigMatrix = birthMatrix + migrationInMatrix - migrationOutMatrix
    
    return (birthMigMatrix, deathMatrix)
