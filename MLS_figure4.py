...
def plot_verFracl(axs, n0, mig_rel_vec):
    ...
    #colors = [ mpl.cm.plasma(x) for x in np.linspace(0.1, 0.8, len(mig_rel_vec)) ]
...
#           ncol=4, mode="expand", borderaxespad=0.)
    axs.legend(bbox_to_anchor=(1.05, 0), loc='lower left', borderaxespad=0., borderpad=0.)
...
def plot_tauH_heatmap(fig, axs, cbaxes):
    ...
    cb = fig.colorbar(axl, ax=cbaxes, orientation='horizontal', \
                 ...
                 ticks=[-6, -3, 0, 3, 6, 9], anchor = (0.5, 0), aspect=30, shrink=0.8)
...
def create_fig():
    ...
    ax.annotate('$\\frac{\\theta/\\beta}{n_0/k}=$',
            xy=(w1+lm+0.02, bm+h), xycoords='figure fraction',
