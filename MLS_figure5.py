...
def plot_helper_frac(axs, cost_vec):
    ...
    axs.legend(handles=handle_list, loc='upper right')
...
def plot_tau_var_rel(axs, data1D):
    colors = sns.color_palette("Blues_d", n_colors=len(sigma_vec))
    for i, sigma in enumerate(sigma_vec):
        x, yS = select_data(data1D, 1, i)
        x, yNS = select_data(data1D, 0, i)
        yRel = yS / yNS
        axs.plot(x, yRel, linewidth=1, c=colors[i],
                 label='$%.2f$' % sigma)

    axs.set_xlabel('$\\log_{10} \\frac{tau_{var}}{\\tau_H}$')
    axs.set_ylabel("fold increase $\\langle f \\rangle$")
    maxY = 20
    xStep = 3
    yStep = 3
...
def create_fig():
    ...
    ax.annotate('$\\gamma=$',
                xy=(w+lm-0.1, bm+h), xycoords='figure fraction',
...
#    ax = fig.add_axes([lm + w + cmh, bm, w, h])
#    plot_tau_var_rel(ax, data1D)
