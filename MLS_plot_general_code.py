...
def calc_tauH(n0, theta, cost):
    ...
    lown = np.log( (k * theta) / ((k + n0) * theta - n0 * alpha) ) / (alpha - theta)
    #approximation for n0 > n0_crit  
    highn1 = np.log( k * (alpha + theta) / (2 * (n0 * (alpha - theta) + k * theta)) ) 
    highn2 = np.log( (n0 * (alpha - theta) + k * theta) / (2 * (n0 * (alpha + theta))) ) 
...
def data_ticks(dataRange, step):
    nNeg = np.floor((0-dataRange[0]) / step)
    nPos = np.floor(dataRange[1] / step)
