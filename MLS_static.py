...
def calc_max_group_num(K_H, B_H, D_H):
    carCap = K_H * (1 + B_H) / (1 - D_H)
    return carCap
...
def calc_max_host_prop(K_H, B_H, tau_H, host_nr):
    # max birth rate at c=1
    maxBirthRate = (1 + B_H)
    # max death rate at c=0
    maxDeathRate = host_nr / K_H
...
def calc_max_num_timesteps(model_par, dtVec):
    ...
    maxHostNum = calc_max_group_num(K_H, B_H, D_H)
    # add some room for noise, assume noise goes as 1/sqrt(N)
    maxHostNum = int(np.ceil(maxHostNum*(1 + 3 / math.sqrt(maxHostNum))))

    # calculate highest ever host propensity
    maxHostProp = calc_max_host_prop(K_H, B_H, TAU_H, maxHostNum)
    ...
    return (Num_t, Num_t_sample, dt)

# calculates time step to use to keep max 1 host event per step for current nr of hosts


@jit(f8(f8, f8, f8, f8, f8[::1]), nopython=True)
def calc_dynamic_timestep(K_H, B_H, tau_H, curr_host_num, dtVec):
    # calculate highest ever host propensity
    maxHostProp = calc_max_host_prop(K_H, B_H, tau_H, curr_host_num)
    # calculate smallest time step ever needed
    dt = mlsg.calc_max_time_step(maxHostProp, dtVec)
    return dt
...
@jit(UniTuple(f8, 2)(f8, f8, f8, f8))
def host_birth_composition_fixedvar(fracPar, n0, sigma, rnd):
    # draw fraction from truncated normal distribution
    fracOff = mlsg.trunc_norm_fast(fracPar, sigma, 0, 1, rnd)
...
def host_birth_event(c, d, randNum, model_par):
    ...
    id_group = select_host_to_reproduce(c, randNum[0], B_H)
    ...
    if model_par['sampling'] == "conthypogeo":
        ...
        fracPar = np.asscalar(c[id_group] / (c[id_group] + d[id_group]))
        cOff, dOff = host_birth_composition_fixedvar(
            fracPar, n0, sigma, randNum[1])
...
def host_birth_death_propensity(c, d, dt, B_H, D_H, TAU_H, K_H):
    ...
    totBirthProp = (dt / TAU_H) * (NumHost + B_H * totC)
    totDeathProp = (dt / TAU_H) * (NumHost / K_H) * (NumHost - D_H * totC)
...
def run_model_fixed_parameters(model_par):
    ...
    # calc timesteps
    Num_t, Num_t_sample, dt = calc_max_num_timesteps(model_par, dtVec)
    ...
    # init randNum
    numTStart = min(maxRandMatSize, Num_t)
    rndMat = mlsg.create_randMat(numTStart, 3)
    ...
    # get first time step
    dt = calc_dynamic_timestep(K_H, B_H, TAU_H, CVec.size, dtVec)
    ...
    while currT <= model_par['maxT']:
        ...
        if rndMat[tiR, 0] < birthProp:
            # process host birth event
            CVec, DVec = host_birth_event(
                CVec, DVec, rndMat[tiR, 1:3], model_par)
            # update time step to new number of groups
            dt = calc_dynamic_timestep(K_H, B_H, TAU_H, CVec.size, dtVec)
        elif rndMat[tiR, 0] < (birthProp+deathProp):
            # process host death event
            CVec, DVec = host_death_event(
                CVec, DVec, rndMat[tiR, 1], model_par)
            # update time step to new number of groups
            dt = calc_dynamic_timestep(K_H, B_H, TAU_H, CVec.size, dtVec)
        ...
        if tiR >= maxRandMatSize:
            rndMat = mlsg.create_randMat(maxRandMatSize, 3)
            tiR = 0
