    ...
    for tt in range(numSubStep):
        ...
        if rndMat[ridx, 0] < cumulPropVec[-1]:
            ...
            if birthEvent and hostAlive:
                ...
                cOff = n0 * fracOff
                dOff = n0 * (1 - fracOff)
                # cOff, dOff = mlss.host_birth_composition_fixedvar(
                #     fracPar, n0, sigma, rndMat[ridx, 1])
...
def run_model_fixed_parameters(model_par):
    ...
    # init groups
    CVec, DVec = mlss.init_comm(model_par)
    ...
    while currT <= maxT:
        ...
        mlss.update_comm(CVec, DVec, cost, mu, mig, dtBac)
        ...
        if currT >= nextSampleT:
            ...
            lowError = Output['rms_err'][sampleIndex - 1] < model_par['rms_err_treshold']
...
def single_run_with_plot(MODEL_PAR):
    ...
    # run code
    start=time.time()
    Output, InvestmentPerHost, FracVec, AgeVec = run_model_fixed_parameters(MODEL_PAR)
    end=time.time()
    print("Elapsed time run 1 = %s" % (end - start))
